...
int main(int argc, char** argv) {
  ...
  Body ibody[N/size], jbody[N/size], tmp[N/size];    //add tmp
  srand48(rank);
  ...
  MPI_Win win;    //add
  MPI_Win_create(tmp, N/size*sizeof(Body), sizeof(Body), MPI_INFO_NULL, MPI_COMM_WORLD, &win);    //add
  for(int irank=0; irank<size; irank++) {
    ...
    MPI_Win_fence(0, win);    //add
    MPI_Put(jbody, N/size, MPI_BODY, send_to, 0, N/size, MPI_BODY, win);    //add
    MPI_Win_fence(0, win);    //add
    memcpy(&jbody, &tmp, N/size*sizeof(Body));    //add
